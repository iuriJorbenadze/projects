...
public class Yahtzee_extension extends GraphicsProgram implements YahtzeeConstants {
	...
	public void run() {
		...
		// game is in while loop so game goes on
		// until player do not close application window
		while (true) {
	...
	/** extension methods bellow */

	// when game finishes tells to click the screen, and waits untill he clicks
	private void proceedToNextPart() {
		GLabel clickToContinue = new GLabel("Click To Continue!");
		clickToContinue.setFont("Times New Roman-20");

		// add label in free space in bottom right corner
		add(clickToContinue, getWidth() - 200, getHeight() - 15);
		waitForClick();
	}
	...
	private void gameOver() {
		for (int i = 0; i < 4; i++) {
			...
			add(gameOver, (getWidth() - gameOverW) / 2, (getHeight() - gameOverH) / 2);
			pause(500);
		}
	}

	// this method tells plater to click the screen, after click method removes
	// everything on screen
	// and after, next iteration of while cycle begins
	private void restartGame() {
		// println("end");
		removeAll();
		GLabel restartGameLabel = new GLabel("Click To Start Again!");
		restartGameLabel.setFont("Times New Roman-50");

		add(restartGameLabel, getWidth() / 2 - restartGameLabel.getWidth() / 2,
				getHeight() / 2 - restartGameLabel.getHeight() / 2 + 25);
